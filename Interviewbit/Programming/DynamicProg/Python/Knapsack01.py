# -*- coding: utf-8 -*-
"""
Created on Thu Jan 29 12:27:05 2026
...
def solve(A, B, C):
        ...
        n_A = len(A)
        min_weight = min(B)
        items = list(range(n_A))
        def helper(items_remain, wg_remain, memo = {}):
            n_ongoing = len(items_remain)
            if (n_ongoing, wg_remain) in memo:
                return memo[(n_ongoing), wg_remain]
            if n_ongoing == 0 or wg_remain < min_weight:
                return (0, ())
            cur_item = items_remain[0]
            cur_item_val = A[cur_item]
            cur_item_wg  = B[cur_item]
            if cur_item_wg > wg_remain:   #only right
                result = helper(items_remain[1:], wg_remain, memo)
            else:
                # left
                withCurVal, withCurTake = helper(items_remain[1:], wg_remain - cur_item_wg, memo)
                withCurVal += cur_item_val
                
                # right
                woCurVal, woCurTake = helper(items_remain[1:], wg_remain, memo)
                
                # Choose
                if withCurVal > woCurVal:
                    result = (withCurVal, withCurTake + (cur_item, ))
                else:
                    result = (woCurVal, woCurTake)
            memo[(n_ongoing, wg_remain)] = result
            return result
        return helper(items, C)[0]

# issue: RecursionError: maximum recursion depth exceeded while calling a Python object interviewbit
...
# B : [ 8, 9, 10, 5, 2, 6, 7, 7, 7, 7, 7, 8, 2, 5, 1, 7, 2, 3, 2, 5, 10, 6, 3, 4, 5, 2, 6, 3, 4, 2, 7, 9, 9, 3, 8, 8, 2, 3, 7, 1, 5, 10, 5, 7, 1, 4, 7, 3, 5, 6, 9, 9, 2, 3, 2, 5, 10, 9, 3, 5, 6, 3, 10, 10, 9, 4, 9, 7, 10, 9, 7, 7, 3, 4, 9, 3, 7, 3, 8, 6, 8, 9, 3, 8, 9, 1, 3, 1, 9, 10, 3, 9, 3, 1, 6, 6, 3, 8, 7, 8, 9, 8, 9, 2, 7, 1, 10, 6, 9, 10, 1, 1, 1, 8, 3, 6, 6, 3, 2, 5, 9, 7, 5, 7, 9, 10, 9, 4, 6, 6, 2, 6, 6, 9, 5, 1, 6, 5, 8, 3, 2, 5, 10, 10, 1, 4, 1, 4, 6, 1, 8, 7, 8, 9, 4, 7, 5, 5, 2, 10, 6, 6, 8, 5, 8, 4, 5, 1, 9, 9, 2, 6, 3, 4, 3, 5, 7, 7, 7, 3, 10, 1, 5, 10, 4, 2, 2, 10, 10, 6, 6, 6, 7, 1, 6, 10, 5, 7, 4, 5, 10, 1, 6, 4, 10, 2, 3, 9, 9, 2, 9, 7, 1, 3, 5, 10, 1, 3, 3, 2, 6, 5, 5, 5, 3, 6, 1, 5, 3, 2, 3, 8, 8, 10, 6, 6, 5, 1, 4, 5, 2, 3, 3, 10, 9, 3, 3, 8, 9, 6, 9, 4, 10, 5, 5, 7, 
 3, 7, 2, 8, 1, 2, 6, 3, 3, 10, 1, 6, 2, 9, 4, 7, 9, 9, 7, 2, 7, 7, 8, 1, 5, 10, 9, 10, 5, 10, 9, 8, 6, 7, 5, 9, 7, 9, 8, 9, 1, 10, 6, 7, 10, 1, 1, 2, 4, 6, 6, 9, 1, 2, 8, 8, 5, 7, 8, 1, 8, 5, 2, 3, 2, 9, 4, 6, 8, 2, 5, 8, 7, 9, 3, 4, 1, 3, 8, 9, 9, 6, 10, 8, 2, 4, 5, 8, 2, 4, 8, 2, 3, 8, 6, 4, 5, 5, 6, 6, 2, 5, 2, 3, 7, 7, 4, 4, 1, 1, 9, 2, 4, 9, 2, 7, 10, 9, 1, 2, 4, 10, 4, 10, 4, 7, 6, 6, 5, 1, 6, 1, 7, 6, 6, 2, 9, 10, 7, 3, 3, 6, 1, 6, 3, 6, 3, 1, 7, 8, 3, 9, 1, 1, 7, 9, 4, 1, 7, 5, 4, 8, 1, 6, 3, 1, 8, 6, 7, 2, 9, 2, 9, 6, 7, 6, 10, 3, 7, 6, 3, 4, 3, 2, 3, 3, 9, 9, 9, 8, 5, 1, 1, 6, 10, 10, 6, 4, 1, 10, 10, 6, 2, 2, 2, 3, 4, 4, 10, 8, 3, 4, 4, 1, 5, 9, 5, 4, 10, 3, 5, 10, 1, 6, 8, 5, 8, 7, 7, 4, 1, 10, 1, 9, 1, 2, 6, 2, 1, 4, 10, 6, 9, 10, 7, 3, 6, 8, 7, 4, 9, 6, 10, 4, 1, 3, 6, 2, 6, 8, 8, 10, 5, 2, 8, 9, 6, 8, 9, 9, 5, 3, 4, 9, 5, 2, 4, 1, 5, 2, 8, 8, 3, 2, 4, 2, 4, 2, 6, 2, 6, 1, 6, 2, 3, 3, 2, 6, 6, 4, 9, 10, 3 ]
# C : 609
...
int knapsack(vector<int> &A, vector<int> &B, int n, int c, vector<vector<int>> &dp){
    if(n==0 || c==0){
        return 0;
    }
    if(dp[n][c]!=-1)
...
class Solution {
    def solve(values: Array[Int], wt: Array[Int], W: Int): Int = {
        val dp = Array.ofDim[Int](values.length + 1, W + 1)
        for (i <- 1 to values.length) {
          for (w <- 0 to W) {
            dp(i)(w) = dp(i - 1)(w)
            if (wt(i - 1) <= w) {
              dp(i)(w) = dp(i)(w) max (dp(i - 1)(w - wt(i - 1)) + values(i - 1)) max dp(i)(w - 1)
            }
          }
        }
    ...
    for i := 1; i <= n; i++ {
        for j := 1; j <= C; j++ {
            if B[i-1] > j {
                ...
                res[i][j] = max(res[i-1][j-B[i-1]] + A[i-1], res[i-1][j])
            }
        }
    }
    return res[n][C]
